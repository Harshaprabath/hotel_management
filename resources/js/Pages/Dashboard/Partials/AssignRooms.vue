<template>
    <AppLayoutVue>
      <div class="cards p-3 py-4 cards_custom">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Assign Rooms</h2>
        <br />
        ...
        <ul class="nav nav-tabs">
          ...
          <li>
            <a data-toggle="tab" class="nav-link" href="#menu2">Using Rooms</a>
          </li>
        </ul>
        <br />
        <div class="tab-content">
            ...
            <table class="table table-bordered" id="dataTable1" width="100%" cellspacing="0">
              <thead>
                <tr>
                  <th>Room Number</th>
                  <th>Price</th>
                  <th>Assign Customers for Rooms</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <form @submit.prevent="submit">
                    ...
                    <div class="modal-body">
                      <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                          <JetInput
                            id="noOfMembers"
                            v-model="assignRoomform.noOfMembers"
                            type="number"
                            class="form-control form-control-user"
                    ...
                    <div class="modal-footer">
                      ...
                      <button
                        ...
                        :class="{ 'opacity-25': assignRoomform.processing }"
                        :disabled="assignRoomform.processing"
                      >Save</button>
                    </div>
                  </form>
                </div>
          ...
          <div id="menu2" class="tab-pane fade">
            ...
            <table class="table table-bordered" id="dataTable2" width="100%" cellspacing="0">
              ...
              <tbody>
                <tr v-for="room in usingrooms" :key="room.id">
                  <td>{{room.number}}</td>
                  <td>Rs {{room.price}}/=</td>
                  <td>{{room.customer}}</td>
                  ...
                  <td>
                    <button
                      ...
                      @click.prevent="out(room.id)"
                    >
                      <i  style="color: red">Out</i>
                    </button>
                  </td>
                </tr>
              </tbody>
  ...
  export default {
    ...
    methods: {
      getRoom(room) {
        console.log(room);
        this.assignRoomform.roomid = room.id;
      },
      submit() {
        const modalToggle = document.getElementById("toggleMyModal");
        console.log(this.addBookingform);
        this.assignRoomform.post(route("assingroom.store"), {
          onSuccess: () => {
            this.assignRoomform.reset();
            Swal.fire({
              position: "top-scroll-to-top",
              icon: "success",
              title: "Room has been Assign for Use ",
          ...
          onError: () => {
            Swal.fire({
              position: "top-scroll-to-top",
              icon: "error",
              title: "error while assignning",
              showConfirmButton: false,
              timer: 1500
            });
          },
          onFinish: () => {
            const modalToggle = document.getElementById("AssignRoomModal");
      ...
      out(id) {
        ...
        try {
          ...
          Inertia.post(route("assingroom.destroy", { id: id }));
          Swal.fire({
            position: "top-scroll-to-top",
            icon: "success",
            title: "Success",
